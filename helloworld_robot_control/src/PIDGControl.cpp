...
namespace tum_ics_ur_robot_lli {
    namespace RobotControllers {
        PIDGControl::PIDGControl(double weight, const QString &name):
        ...
        oldTime(0.0)
        {
            pubCtrlData=n.advertise<tum_ics_ur_robot_msgs::ControlData>("helloworld/PIDControl/ControlData",100);
            goalSetterService = n.advertiseService("helloworld/PIDControl/setTrajectory", &PIDGControl::setNewTrajectoryCallback, this);
        }

        PIDGControl::~PIDGControl() {}

        void PIDGControl::setQInit(const JointState& qinit)
        {
            m_qInit=qinit;
        }
        void PIDGControl::setQHome(const JointState& qhome)
        {
            m_qHome=qhome;
        }
        void PIDGControl::setQPark(const JointState& qpark)
        {
            m_qPark=qpark;
        }


        bool PIDGControl::init() {

            std::string currentParamPath = "~PIDControl";
        ...
        bool PIDGControl::start(){
            running = true;
            return true;
        }

        bool PIDGControl::stop(){
            running = false;
            m_startFlag = false;
            return true;
        }

        Vector6d PIDGControl::update(const RobotTime& time, const JointState &current){
        ...
        Eigen::Matrix<double,6,6> PIDGControl::Jacobian(Vector6d q, Vector6d l){
        ...
        Eigen::Matrix<double,4,4> PIDGControl::Transform_EF_W(Vector6d q, Vector6d l){
        ...
        bool PIDGControl::setNewTrajectoryCallback(helloworld_robot_control::setTrajectoryGoal::Request &req,
            ...
            if(req.goal.size() != 6){
                ...
                goalPosString << "]";
                ROS_ERROR_STREAM("PIDControl: cannot set goal to " << goalPosString.str() << " ! Please provide exactly 6 joint angles!");
                resp.success = false;
                return false;
            }
            Vector6d goal;
        ...
        bool PIDGControl::setNewTrajectory(Vector6d goal, double trajectoryDuration, bool identification) {
            nextGoal = goal;
            nextDuration = trajectoryDuration;
            return true;
        }

        bool PIDGControl::isRunning() {
            return running;
        }
