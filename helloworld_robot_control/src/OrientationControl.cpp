...
namespace tum_ics_ur_robot_lli {
    namespace RobotControllers {
        OrientationControl::OrientationControl(double weight, const QString &name):
        ...
        oldTime(0.0)
        {
            pubCtrlData=n.advertise<tum_ics_ur_robot_msgs::ControlData>("helloworld/OrientationControl/ControlData",100);
            goalSetterService = n.advertiseService("helloworld/OrientationControl/setTrajectory", &OrientationControl::setNewTrajectoryCallback, this);
            assumeArucoPositionService = n.advertiseService("helloworld/OrientationControl/assumeArucoPosition", &OrientationControl::doArucoRequest, this);
        }

        OrientationControl::~OrientationControl() {}

        void OrientationControl::setQInit(const JointState& qinit)
        {
            m_qInit=qinit;
        }
        void OrientationControl::setQHome(const JointState& qhome)
        {
            m_qHome=qhome;
        }
        void OrientationControl::setQPark(const JointState& qpark)
        {
            m_qPark=qpark;
        }


        bool OrientationControl::init() {

            std::string currentParamPath = "~OrientationControl";
            ...
            //movement duration to aruco
            currentParam="/arucoMovementDuration";
            ros::param::get(currentParamPath+currentParam,params);
            if(params.size()!=1){
                std::string errorString = this->name().toStdString() + "bad number of movement duration to aruco parameters: " + std::to_string(params.size()) + "using default from header";
            } else {
                arucoMovementDuration = params[0];
            }
        ...
        bool OrientationControl::start(){
            running = true;
            return true;
        }

        bool OrientationControl::stop(){
            running = false;
            m_startFlag = false;
            return true;
        }

        Vector6d OrientationControl::update(const RobotTime& time, const JointState &current){
...
//            std::cout << TEF_W << std::endl;
            ...
            if((time.tD() - currentPathBeginTime) > m_totalTime && nextDuration > 0.0){
                ...
                currentPathBeginTime = time.tD();
                nextGoal = Vector6d::Zero();
                std::cout << nextDuration << std::endl;
                std::cout << "#################################################################" << std::endl;
                nextDuration = 0.0;
            ...
            lambdad_v.append(trajectory_vec6[0].block<3,1>(3,0));
            lambdad_v.append(trajectory_vec6[1].block<3,1>(3,0));
            lambdad_v.append(trajectory_vec6[2].block<3,1>(3,0));

            // Calulate Position and Orientation Error of EF
            Vector3d DeltaX;
            Vector3d DeltaLambda;
            DeltaX = Xd_vec[0] - X_vec;
            DeltaLambda = lambdad_v[0] - lambda;
            ...
            Xpr_W = Xd_vec[1] + m_Kp*DeltaX + m_Ki*error;
...
//            }
            ...
            Vector3d Lambdapr_W;
            Vector3d Lambdapr_0;
            Lambdapr_W = lambdad_v[1] + KpLambda*DeltaLambda;//+ KiLambda*error1;
...
//            publish(time.tD(),(MatrixXd(6,1) << lambda4,Vector2d::Zero()).finished(),(MatrixXd(6,1) << (JLambda*JW*current.qp),Vector3d::Zero()).finished(), Vector6d::Zero(), (MatrixXd(6,1) << lambdad_v[0],Vector3d::Zero()).finished(), (MatrixXd(6,1) << Xd_vec[1],Vector3d::Zero()).finished(), (MatrixXd(6,1) << DeltaX,Vector3d::Zero()).finished(), S, Tau);
            publish(time.tD(), (MatrixXd(6,1) << X_vec, lambda).finished(),(MatrixXd(6,1) << Xpr_W, Lambdapr_W).finished(), Vector6d::Zero(), (MatrixXd(6,1) << Xd_vec[0], lambdad_v[0]).finished(), (MatrixXd(6,1) << Xd_vec[1],lambdad_v[1]).finished(), (MatrixXd(6,1) << DeltaX, DeltaLambda).finished(), S, Tau);
...
//                std::cout << "########################" << std::endl;
        ...
        Eigen::Matrix<double,6,6> OrientationControl::Jacobian(Vector6d q, Vector6d l){
        ...
        Eigen::Matrix<double,3,3> OrientationControl::JacobianLambda(Vector4d lambda){
        ...
        Eigen::Matrix<double,4,4> OrientationControl::Transform_EF_0(Vector6d q, Vector6d l){
        ...
        bool OrientationControl::setNewTrajectoryCallback(helloworld_robot_control::setTrajectoryGoal::Request &req,
            ...
            if(req.goal.size() != 6){
                ...
                goalPosString << "]";
                ROS_ERROR_STREAM("OrientationControl: cannot set goal to " << goalPosString.str() << " ! Please provide exactly 6 joint angles!");
                resp.success = false;
                return false;
            }
            Vector6d goal;
        ...
        bool OrientationControl::setNewTrajectory(Vector6d goal, double trajectoryDuration, bool identification) {
            nextGoal = goal;
        ...
        bool OrientationControl::isRunning() {
            return running;
        }

        bool OrientationControl::doArucoRequest(helloworld_robot_control::ArucoRequest::Request &req, helloworld_robot_control::ArucoRequest::Response &resp){
            int arucoNumber = req.arucoNumber;
            if(arucoNumber < 0) return false;
            int msgMarkerNumber = -1;
            boost::shared_ptr<cv_tracker::destination_msg const> goalMsg;
            while(msgMarkerNumber != arucoNumber){
                goalMsg = ros::topic::waitForMessage<cv_tracker::destination_msg>("helloworld/ef_destination", n, ros::Duration(10,0));
                if(!goalMsg) {
                    resp.success = false;
                    return false;
                }
                msgMarkerNumber = goalMsg->frame_index;
            }

            Vector6d goal;
            goal << goalMsg->x, goalMsg->y, (goalMsg->z+0.30), goalMsg->rx, goalMsg->ry, goalMsg->rz;
            resp.success = this->setNewTrajectory(goal, arucoMovementDuration, 'false');
            return resp.success;
        }
