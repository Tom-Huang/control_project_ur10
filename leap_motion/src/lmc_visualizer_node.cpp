...
int main(int argc, char** argv) {
    ...
    nh1.getParam("/enable_filter", enable_filter);
    ros::Subscriber human_sub;

    human_sub = nh1.subscribe<leap_motion::Human>("leap_device", 1, frameCallback);
    if(enable_filter)
    {
        human_sub = nh1.subscribe<leap_motion::Human>("leap_filtered", 1, frameCallback);
    }

    ROS_INFO("enable_filter: %s", enable_filter ? "true" : "false");
    
    ros::Publisher m_pub = nh1.advertise<visualization_msgs::MarkerArray>("visualization_marker_array", 1);
    marker_pub = &m_pub;
